...
#   | |                                                                                                | |
#   | |                                         Byte size: 84836                                       | |
#   | |           Checksum: a0f2effef69b4ab5b58b1e1f13a1f162 File name: Talos_v30_a0f2e.bash           | |
...
#   | |      8 - CHECK_VOS - Checks the status of NOS                                                  | |
	...
	then
		...
		TALOS () {
			...
			SETUP_LOG () {
				...
				sshpass -p '**********' ssh -q -o StrictHostKeyChecking=no -t root@192.168.1.100 mkdir -p /root/HW_Logs/"$CDAT"/"$VIGSN"/ > /dev/null
				sshpass -p '**********' rsync -e "ssh -q -o StrictHostKeyChecking=no" -arvc "$LPTH"/"$CDAT"/ root@192.168.1.100:/root/HW_Logs/"$CDAT"/"$VIGSN"/ > /dev/null
			...
			NOS_STATS () {
				...
				VIGSV=$(awk -F'[=|;]' '/Server/{print $11}' /tiger/log/status* | tail -n1)
				VIGPV=$(awk -F'[=|;]' '/Server/{print $13}' /tiger/log/status* | tail -n1)
				VIGCL=$(awk -F'[:|;|"]' '/value/ {print tolower($3)}' /tiger/registry/_showtracker_StationCallLetters.regdef)
				VIGAV=$(/tiger/scripts/getregvalue.sh /av2009/av2009/SW_Image | cut -b 3-19)
				VIGST=$(/tiger/scripts/getregvalue.sh /status/callback/Whale/HostList | cut -c -4)
				VIGSN=$(/tiger/scripts/getregvalue.sh /system/HW_SerialNumber)
				VIGSS=$(echo "$VIGSN" | cut -d "-" -f 3)
				...
				CHECK_NOS () {
					...
					echo $TIME
					echo "| NOS Config:           ${VIGOS}"
					echo $LINS
					echo "| Call Letters:         ${VIGCL^}"
					echo "| Serial:               ${VIGSN}"
					echo "| OS Version:           ${VIGSV}"
					echo "| OS Patch:             ${VIGPV}"
					echo "| AV HEX File:          ${VIGAV}"
					echo "| Whale Stack:          ${VIGST^}"
			...
			LOG_STATS () {
				...
				f=0
					...
					for ser in secondary tertiary quaternary
					do

						export SATER_"$a"="$(grep -E -i 'lock-loss|periodical' /tiger/log/error_2019*  | grep -c $ser | tr -d "[:space:]" | wc -m)"
						((a++))

					done

				LODAY=$(grep -E -ic '(periodical.*max-TEI=[1-9]|lock-loss)' /tiger/log/error* | cut -b 18-25 | tail -n1 | date '+%m/%d')
				LOCOU=$(grep -E -ic '(periodical.*max-TEI=[1-9]|lock-loss)' /tiger/log/error* | cut -b 31-35 | tail -n1)

				b=1

					for dme in warn err crit alert emerg
					do

						export DM_"$b"="$(dmesg -l $dme | wc -l | xargs printf "%3s\n")"
						((b++))

					done

				SHUTD=$(grep -E -i 'shut|unexpected' /tiger/log/records_0000* | wc -l)
				HEADR=$(echo "tiger Logs:")
				DMESG=( $DM_1 $DM_2 $DM_3 $DM_4 $DM_5 )
					...
					do
						...
						then
							...
							if [[ $e -ge 2 && ${DMESG[$e]} -ge 1 || $e = 1 && ${DMESG[$e]} -ge 15 || $e = 0 && ${DMESG[$e]} -gt 200 ]]
							then
					...
					if [[ $SATER_1 && $SATER_2 && $SATER_2 -le 5 ]] && [[ $f = 0 ]]
					then

						export LOGCH="$(echo "Within Range")"

					else

						export LOGCH="$(echo "Check Logs")"

					fi

				LOG_1=$(echo "$DM_5            Lock S:  $SATER_1")
				LOG_2=$(echo "$DM_4            Lock T:  $SATER_2")
				LOG_3=$(echo "$DM_3            Lock Q:  $SATER_3")
				LOG_4=$(echo "$DM_2            Shut D:  $SHUTD")
				LOG_5=$(echo "$DM_1            $LODAY:  $LOCOU")
			...
			AWS_STATS () {
				...
				sshpass -p '**********' ssh -o StrictHostKeyChecking=no -t root@192.168.1.100 python orka 2> /dev/null | grep "$VIGSN" > "$FPTH"/Whale.txt
				ORSCA=$(grep -c "$VIGSN" "$FPTH"/Whale.txt)
					...
					then
						...
						ORSTO=$(sshpass -p '**********' ssh -o StrictHostKeyChecking=no -t root@192.168.1.100 python orka 2> /dev/null | wc -l)
			...
			paste <("CHECK_LOG") <("CHECK_STO") <("CHECK_CON") <("CHECK_CLN") | column -s $'\t' -t
